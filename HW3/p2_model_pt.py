...
class Block(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #     nn.Linear(cfg.n_embd, 196), 
        #     nn.GELU(approximate='tanh'),
        #     nn.Linear(196, cfg.n_embd),
        #)
...
class Decoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #self.pfte = nn.Embedding(cfg.vocab_size, cfg.n_embd)
        #self.pfpe = nn.Embedding(8, cfg.n_embd)
        self.soft_emb = nn.Embedding(768, cfg.n_embd)
        ...
        #self.pfte.weight = self.transformer.wte.weight
        #print(self.pfte.weight)
    ...
    def forward(self, img_feat, x, prompt):
        x = torch.narrow(x, 1, 0, min(x.size(1), self.block_size))
        #x_pre = torch.cat((prompt, x), dim = 1)
        #print(prompt.shape)
        #x_pre = torch.narrow(x_pre, 1, 0, min(x_pre.size(1), self.block_size))
        #x = self.pfpe(x_pre)
        pos = torch.arange(x.size()[1], dtype=torch.long, device=x.device).unsqueeze(0)

        #t_prompt = torch.narrow(prompt, 1, 0, min(prompt.size(1), 7))
        #prompt = torch.arange(prompt.size()[1], dtype=torch.long, device=prompt.device).unsqueeze(0)
        prompt = torch.narrow(prompt, 1, 0, min(prompt.size(1), self.block_size))
        
        #p = self.pfte(t_prompt) + self.pfpe(pos_prompt)
        #print(prompt.shape)
        p = self.soft_emb(prompt)
...
if __name__ == '__main__':
    ...
    model = PT_Transfomer(encoder, decoder_checkpoint)
    #model2 = PT_Transfomer(encoder, decoder_checkpoint)
    #dat = dataset[0]
    ...
    '''prompt = 'The caption of the image is '
    prompt_n = tokenizer.encode(prompt)
    prompt_n = torch.LongTensor(prompt_n)
    prompt_n = prompt_n.unsqueeze(0)
    prompt_n = prompt_n.repeat(1, 1)'''
    #prompt_n = torch.FloatTensor(10, 768).uniform_(-0.5, 0.5)
    #prompt_n = torch.randint(0, 768, (10, 1))
    prompt = torch.randint(low = 0, high =768, size = (8, 10))
    print(prompt.shape)
    prompt_n = prompt
    prompt_n = prompt_n.unsqueeze(0)
    t = model(im, capr, prompt_n)
    print(t.shape)
