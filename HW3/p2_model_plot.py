...
class Cross_Attention(nn.Module):
    ...
    def forward(self, img_feat, text_feat):
        ...
        att = F.softmax(att, dim=-1)
        #attn = self.img_attn.weight
        return self.c_proj((att @ v).transpose(1, 2).contiguous().view(B, T, C)), attn  # [b, 1, 1024, 196+1] @ [b, 1, 196+1, 768/1].trans.view.proj => [b, 1024, 768]
...
class Block(nn.Module):
    ...
    def forward(self, img_feat, x):
        ...
        c_out, attn = self.cross_attn(img_feat, self.ln_3(x))
...
class Decoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #self.img_attn = nn.Linear(1024, cfg.n_embd)
        #self.pfte = nn.Embedding(cfg.vocab_size, cfg.n_embd)
        #self.pfpe = nn.Embedding(2, cfg.n_embd)
    ...
    def forward(self, img_feat, x):
        ...
        for layer in self.transformer.h:
            if layer == self.transformer.h[6]:
                x, attn = layer(img_feat, x)
            else:
                x, _ = layer(img_feat, x)                
        x = self.lm_head(self.transformer.ln_f(x))
        return x, attn
class LORA_Transfomer(nn.Module):
    ...
    def forward(self, img, cap):
        ...
        cap_out, attn = self.decoder(img_feat, cap)
        return cap_out, attn
