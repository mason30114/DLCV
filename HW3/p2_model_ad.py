...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        out_prev = self.c_proj((att @ v).transpose(1, 2).contiguous().view(B, T, C))
        out = self.adapter(out_prev)
        return out
...
if __name__ == '__main__':
    ...
    model1 = AD_Transfomer(encoder, decoder_checkpoint)
    model2 = AD_Transfomer(encoder, decoder_checkpoint)
    dat = dataset[0]
