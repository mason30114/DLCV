...
# Inference step modified from https://github.com/openai/CLIP
@torch.no_grad()
def inference(model, args, text_inputs):
    #correct = 0
    ...
    for images in tqdm(test_loader):
        model.eval()
        image_input = images.to(args.device)
...
def save_csv(prediction, test_dataset, filepath):
...
if __name__ == '__main__':
    ...
    # Define Dataset
    test_dataset = P1Dataset(args.img_path, transforms = transforms.Compose([transforms.ToTensor()]), mode = 'test')
    ...
    # Start inference
    inference(model, args, text_inputs)
