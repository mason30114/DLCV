    ...
    for epoch in range(args.num_epoch):
        ...
        #for images, caption_m, caption_l, fn in tqdm(valid_loader):
            ...
            with autocast():
                ...
                #prompt = torch.FloatTensor(10, 768).uniform_(-0.5, 0.5)
                prompt = torch.randint(low = 0, high =768, size = (caption_m.shape[0], 10))
                prompt = prompt.to(args.device)
                #prompt_n = prompt.unsqueeze(0)
                #prompt = prompt.repeat(caption_m.shape[0], 1)
                '''prompt = args.prompt
                prompt_n = tokenizer.encode(prompt)
                prompt_n.insert(0, 50256)
                prompt_n = torch.LongTensor(prompt_n)
                prompt_n = prompt_n.unsqueeze(0)
                prompt_n = prompt_n.repeat(caption_m.shape[0], 1)
                prompt_n = prompt_n.to(args.device)'''
                #tokenizer = BPETokenizer(encoder_file = args.encoder_fn, vocab_file = args.vocab_fn)
                #caption_l = torch.cat(prompt, caption_l, dim = 1)
                optimizer.zero_grad()
                output = model(images, caption_m, prompt)
                output = output[:, prompt.shape[1]:, :]
                #print(output.shape)
                loss = criterion(output.reshape(-1, 50257), caption_l.view(-1))
        ...
        ckpt_fn =  os.path.join(args.ckpt_dir, 'giant_soft_PT_ep{}.pth'.format(epoch))
        with torch.no_grad():
            model.eval()
        ...
        #cider_score, clip_score = eval_n_save(args, model)
            cider_score, clip_score = eval_n_save(args, model)
            ckpt_fn =  os.path.join(args.ckpt_dir, 'giant_FIN_PT_ep{}_CD{:3f}_CL{:3f}.pth'.format(epoch, cider_score, clip_score))
...
def eval_n_save(args, model):
    ...
    for images, caption_m, caption_l, fn in tqdm(valid_loader):
        ...
        #prompt_n = torch.LongTensor(prompt_n)
        #prompt_n = prompt_n.unsqueeze(0)
        #prompt_n = prompt_n.repeat(args.batch_size, 1)
        #prompt_n = prompt_n.to(args.device)
        #current_word = torch.cat(prompt_n, current_word, dim = 1)
        pred = []
        for current_idx in range(10, args.max_len+10):
            output = model.decoder(img_feat, current_word, prompt)
            output_pred = output.argmax(dim = 2)
            next_input = []
            current_pred = output_pred.squeeze(0)[current_idx].item()
            if current_pred == 50256 or current_idx == args.max_len+9:
...
if __name__ == "__main__":
    ...
    model = PT_Transfomer(args.encoder, args.decoder_checkpoint).to(args.device) 
    ...
    #for name, param in model.decoder.pfte.named_parameters():
    #    param.requires_grad = True 
    for name, param in model.decoder.soft_emb.named_parameters():
        param.requires_grad = True 
    ...
    #load_checkpoint('./p2_lora_ckpt/giant_soft_PT_ep1.pth', model)
    #model.eval()
    #with torch.no_grad():
    #    eval_n_save(args, model)