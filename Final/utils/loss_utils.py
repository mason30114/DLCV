...
def get_losses_with_anchor(config, preds, gts):
    ...
    gt_before_query_replicate = rearrange(gt_before_query.unsqueeze(2).repeat(1,1,N), 'b t N -> (b t N)').to(device)
...
def get_bbox_ratio(hw, device):
    ...
    default_ratios = torch.tensor([0.5, 1, 2], dtype=torch.float32)                                # 3 types of aspect ra
